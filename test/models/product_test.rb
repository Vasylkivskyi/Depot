...
class ProductTest < ActiveSupport::TestCase
  ...
  test 'product price must be positive' do
    product = Product.new(title: 'My book title', description: 'some desc', image_url: 'zzz.jpg')
    product.price = -1
    assert product.invalid?
  ...
  def new_product(image_url)
    Product.new(title: 'Some title', description: 'yyy', price: 1, image_url:)
  end
  ...
  test 'product is not valid without a unique title' do
    ...
    product.invalid?
    assert_equal ['Title should be unique'], product.errors[:title]
  end
end
